...
import { getTaskDetailApi } from '@/app/api/getTaskDetail'
...
export default function ProjectDetail() {
	const { updateIsCreateTask, isEditTask, updateIsEditTask } = useUpdateStatusModal()
	...
	const listMember = projectDetail?.data?.content?.members.map((mem: MemberProject) => {
		return (
			<div key={mem.userId}>
				<Avatar src={`${mem.avatar}`} alt={mem.name ? mem.name : 'Name'} className='h-8 w-8 lg:h-10 lg:w-10' />
			</div>
		)
	})
	...
	// search task
	const dataProjectNameOption = projectDetail?.data?.content?.lstTask?.map((ele: any) => {
		return ele.lstTaskDeTail?.map((ele: any) => {
			return { value: ele.taskId, label: ele.taskName }
		})
	})
															...
															return (
																...
																>
																	<div className=' flex items-center justify-between '>
																		...
																		<div className='opacity-0 group-hover:opacity-100 transition-all duration-200 flex items-start gap-2'>
																			<svg
																				...
																				viewBox='0 0 24 24'
																				fill='none'
																				onClick={() => handleGetTaskDetail(ele?.taskId)}
																			>
																				<path
																					fillRule='evenodd'
																					clipRule='evenodd'
																	...
																	<p className='text-14 md:text-16 leading-1-4 text-neutral-8 font-semibold'>
																		- Priority:{' '}
																		<span className='text-red-1 font-normal'>{ele.priorityTask.priority}</span>
																	</p>
																	<div className='flex items-center gap-1 flex-wrap'>
																		...
																		{ele.assigness.map((member: any) => {
																			return (
																				<div key={member.id}>
																					<Avatar src={member.avatar} alt={member.avatar} />
																				</div>
																			)
																		})}
																	</div>
																	{ele.description.length > 0 && (
																		<p className='text-14 md:text-16 leading-1-4 text-neutral-8 font-semibold flex gap-1 flex-wrap'>
																			- Description:{' '}
																			<span
																				className='text-12 md:text-14 font-normal text-limit-2-line break-all'
		...
		<section className='grid grid-cols-1 gap-3 lg:gap-5'>
			<div className='flex justify-between items-start sm:items-center p-3 rounded-lg  mt-3 md:mt-0 flex-col sm:flex-row bg-white bg-opacity-50'>
				...
				<div className='relative mt-2 md:mt-0'>
					<SelectController
						classNameSelect='lg:min-w-[375px]'
						classNameIcon='!left-0'
						control={control}
						name='taskId'
						options={dataProjectNameOption?.flat()}
						placeholder='Search task...'
						onChange={(selected) => handleGetTaskDetail(+selected)}
						iconSelect={
							<svg
								width='44'
								height='44'
								...
								xmlns='http://www.w3.org/2000/svg'
								style={{
									filter: 'invert(65%) sepia(88%) saturate(426%) hue-rotate(126deg) brightness(86%) contrast(84%)'
								}}
							>
			...
			<div className='grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-5'>{renderCardTaskList()}</div>
