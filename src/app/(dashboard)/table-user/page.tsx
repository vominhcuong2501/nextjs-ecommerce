...
export default function TableUser() {
	...
	const fetchUserEdit = (id: number) => {
		if (userData) {
			...
			const index = arrUserEdit && arrUserEdit?.findIndex((user: any) => id === user.userId)
	...
	const columns: ColumnsProps[] = [
		...
		{
			title: <p className='text-18 md:text-20 text-gradient-red'>Phone number</p>,
	...
	return (
		<section>
			<div className='flex justify-between items-start sm:items-center p-3 rounded-lg mb-3 lg:mb-5 mt-3 md:mt-0 flex-col sm:flex-row bg-white bg-opacity-50'>
				<h1 className='text-24 lg:text-32 text-gradient-red font-bold leading-1-4 '>Management User</h1>
			...
			<div className={`fixed top-0 transition-all duration-200 ${isEditUser ? 'right-0 !z-50' : '-right-[150vw]'}`}>
